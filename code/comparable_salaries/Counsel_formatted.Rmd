---
title: "Legislative Counsel and Comparable City Titles"
output:
  html_document:
    theme: cosmo
    toc: false
...
# ------------------------------------------------------------
# Local civil service title table (avoid network in knit)
...
City Council Legislative Counsel and (in house title) Senior Legislative Counsel roles include the following responsibilities:
...
```{r}
gt::gt(lcp[,.(`Fiscal Year` = fiscal_year, N = n, Level = level, `Median Base Salary` = median_base_salary, `Average Tenure (years)` = round(mean_tenure_years, 2))])  %>% 
  fmt_currency(
...
Bargaining implication: City ranges exceed Council medians at comparable levels; Council Legislative Counsel salaries are not competitive for licensed attorneys performing equivalent work.
