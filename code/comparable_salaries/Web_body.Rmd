...
City Council Web Developers design, build, and maintain web applications that support legislative operations, transparency, and internal workflows. Typical work includes:

-   Full-stack development
-   Accessibility and security practices
-   Deployment and maintenance
-   And collaboration with policy and data staff
...
ggplot(webp, aes(x = fiscal_year, y = median_base_salary)) +
  ...
  labs(
    title = "Median base salary for City Council Web Developers",
...
Computer Specialists (Software) levels II and III

have the most comparable duties to City Council Web Developers, as they are responsible for the following with respect to software systems, websites and applications:
...
Level I is excluded because it can function as an entry/trainee tier, while Council web roles generally require independent delivery and ongoing application ownership. [@nycjobs_css_level2; @nycjobs_css_level3]

```{r}

  cst_range_sub <- cst_range[asg_lvl %in% c("11", "111")]

  cst_range_sub[, `Level (City)` := data.table::fcase(
    asg_lvl == "02", "Level II",
    asg_lvl == "03", "Level III",
    default = asg_lvl
  )]
